...
import { submitCareerApplication } from "../../services/career.api";
...
const CareerForm = () => {
  ...
  const inputBase =
    "w-full rounded-xl border px-5 py-4 text-base border-gray-200 bg-white/80 backdrop-blur-sm shadow-sm focus-visible:border-brand-primary focus-visible:ring-4 focus-visible:ring-brand-primary/20 transition-all";

  return (
    <div className="space-y-8" id="contact-strip">
      ...
      <form onSubmit={handleSubmit} className="space-y-8" noValidate>
        ...
        <div>
          ...
          <input
            ...
            className="block w-full text-sm text-gray-700 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:bg-brand-primary file:text-white file:font-semibold file:shadow-lg hover:file:shadow-xl cursor-pointer border-2 border-dashed border-gray-300 p-8 rounded-2xl hover:border-brand-primary hover:bg-brand-primary/5"
          />
          {errors.resume && (
            <p className="text-sm text-red-500 mt-2">{errors.resume}</p>
          )}
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={handleChange}
            className={`${inputBase} resize-vertical min-h-30`}
            placeholder="Why Negam Care? Your experience..."
          />
        </div>
        ...
        <button
          ...
          className="w-full group bg-linear-to-r from-brand-primary to-brand-accent hover:from-brand-accent hover:to-brand-primary text-white font-bold py-5 px-10 rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 disabled:opacity-50"
        >
